...
export const Chat = () => {
  ...
  return (
    <div className="bg-gray-100 h-3/5 flex flex-col mx-auto ms-3">
      <div className="bg-gray-950 p-4 text-white flex justify-center items-center">
        <span>Support</span>
      </div>
      <div className="flex-1 overflow-y-auto p-4">
        <div className="flex flex-col space-y-2 h-full min-h-[1000px]">
          {messages.map((message, index) => (
            <div key={index} className={`flex ${message.from === 'user' ? 'justify-end' : ''}`}>
              <div className={`p-2 rounded-lg max-w-xs ${message.from === 'user' ? 'bg-slate-800 text-white' : 'bg-gray-300 text-black'}`}>
                {message.text}
              </div>
            </div>
          ))}
